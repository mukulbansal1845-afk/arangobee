...
public class ChangeService {
    ...
    public List<Method> fetchChangeSets(final Class<?> type) throws ArangobeeChangeSetException {
    ...
    private List<Method> filterChangeSetAnnotation(List<Method> allMethods) throws ArangobeeChangeSetException {
        ...
        for (final Method method : allMethods) {
            if (method.isAnnotationPresent(ChangeSet.class)) {
                ...
                if (changeSetIds.contains(id)) {
                    throw new ArangobeeChangeSetException(String.format("Duplicated changeset id found: '%s'", id));
                }
                changeSetIds.add(id);
